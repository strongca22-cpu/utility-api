#!/usr/bin/env python3
"""
OWRS Comparability Migration — Confidence Recalibration

Purpose:
    Patch existing OWRS records in rate_schedules:
    1. Downgrade 1-tier records from "high" to "medium" (Duke criteria:
       high requires tier_count >= 2)
    2. Flag bill outliers for review

    JSONB is already clean — OWRS uses water_rate_to_schedule() helper
    which writes canonical format. No structural fixes needed.

    Bills are calculated from tier structures (not source-primary),
    so no bill normalization is needed — bills are already at true CCF
    benchmarks.
...
Usage:
    python scripts/migrate_owrs_to_comparable.py --dry-run
    python scripts/migrate_owrs_to_comparable.py

Notes:
    - Idempotent: safe to re-run
    - No JSONB fixes (already canonical via water_rate_to_schedule helper)
    - No bill recomputation (bills calculated from tier structure in ingest)
    - 16 budget_based records already at "medium" — no change needed
    - Net effect: 102 records high → medium (1-tier cap)
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="OWRS comparability migration — confidence recalibration"
    )
    parser.add_argument(
...
def recalibrate_confidence(conn, dry_run: bool) -> dict:
    ...
    for row in rows:
        ...
        if changed:
            if new_confidence == "high" and row.confidence != "high":
                stats["upgraded_to_high"] += 1
            elif new_confidence == "medium" and row.confidence == "high":
                stats["downgraded_to_medium"] += 1
            elif new_confidence == "low":
                stats["downgraded_to_low"] += 1
        elif new_confidence == "high":
            stats["high_kept"] += 1
    ...
    logger.info(f"  High kept: {stats['high_kept']}")
...
def main():
    ...
    logger.info("=== OWRS Comparability Migration ===")
