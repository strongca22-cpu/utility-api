#!/usr/bin/env python3
"""
IN IURC Comparability Migration — bill_20ccf Backfill + Outlier Flagging

Purpose:
    Patch existing IN IURC records in rate_schedules:
    1. Backfill bill_20ccf from proportional model (2 × bill_10ccf)
    2. Flag bill outliers for review (if any)

    IURC reports a single bill at 4,000 gallons. The ingest already
...
Usage:
    python scripts/migrate_in_iurc_to_comparable.py --dry-run
    python scripts/migrate_in_iurc_to_comparable.py

Notes:
    - Idempotent: safe to re-run
    - Proportional model: bill_20ccf = 2 × bill_10ccf = 4 × bill_5ccf
    - Original 4,000 gal bill preserved in parse_notes: "Bill @4000gal=$XX.XX"
    - 0 outliers in current dataset (all bills in [5, 200])
"""
...
SOURCE_KEY = "in_iurc_water_billing_2024"


def parse_args():
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(
        description="IN IURC comparability migration — bill_20ccf backfill + outlier flagging"
    )
    parser.add_argument(
...
def backfill_and_flag(conn, dry_run: bool) -> dict:
    ...
    for row in rows:
        ...
        # Update parse_notes with bill_20ccf provenance
...
def main():
    ...
    logger.info("=== IN IURC Comparability Migration ===")
